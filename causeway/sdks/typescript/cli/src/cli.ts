...
program
  .name('raceway')
  .description('Raceway instrumentation CLI')
  .version('0.1.0');

/**
 * Instrument command - automatically add Raceway instrumentation to code
 */
program
  .command('instrument <path>')
  .description('Instrument JavaScript/TypeScript files with Raceway events')
  ...
  .action(async (inputPath: string, options) => {
    console.log('🔍 Raceway Instrumentation\n');
    ...
    if (!options.dryRun && instrumentedCount > 0) {
      console.log(`\n💡 Next steps:`);
      console.log(`   1. Start Raceway server: raceway serve`);
      console.log(`   2. Run your instrumented code`);
      console.log(`   3. View results: raceway tui`);
    }
  });

/**
 * Init command - create raceway configuration
 */
program
  .command('init')
  .description('Initialize Raceway in your project')
  .action(() => {
    console.log('🚀 Initializing Raceway\n');

    // Create .racewayrc.json
    const config = {
    ...
    console.log('✅ Created .racewayrc.json');

    // Check if package.json exists
    if (fs.existsSync('package.json')) {
      const pkg = JSON.parse(fs.readFileSync('package.json', 'utf-8'));

      if (!pkg.dependencies?.['@mode-7/raceway-node']) {
        console.log('\n💡 Add @mode-7/raceway-node to your dependencies:');
        console.log('   npm install @mode-7/raceway-node');
      }

      if (!pkg.devDependencies?.['babel-plugin-raceway']) {
        console.log('\n💡 Add babel-plugin-raceway to your dev dependencies:');
        console.log('   npm install --save-dev babel-plugin-raceway');
      }
    }
    ...
    console.log('   2. Instrument your code: raceway instrument ./src');
    console.log('   3. Start Raceway server: raceway serve');
    console.log('   4. Run your app');
    console.log('   5. View results: raceway tui');
  });

/**
 * Status command - check raceway server status
 */
program
  .command('status')
  .description('Check Raceway server status')
  .option('-u, --url <url>', 'Server URL', 'http://localhost:8080')
  .action(async (options) => {
    console.log('🔍 Checking Raceway server status...\n');
    ...
    try {
      ...
      console.log('\n💡 Start the server with: raceway serve');
    }
  });
